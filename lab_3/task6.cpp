...
const std::vector<int> THRESHOLD = {60, 60, 40};
...
void callBackFunc(int event, int col, int row, int flags, void* userdata) {
    if(event == cv::EVENT_LBUTTONDOWN || event == cv::EVENT_RBUTTONDOWN ) {
        ...
        // Compute the average of the 9x9 points around the clicked one
        ...
        // Create the mask image puttin the color (92, 37, 201) if the values are inside a threshold from average value
        // Put the original image pixel values othewise
        cv::Mat mask(img.rows, img.cols, img.type());
        for(int r = 0; r < img.rows; r++) {
            for(int c = 0; c < img.cols; c++) {
                cv::Vec3b& point = img.at<cv::Vec3b>(r, c);
                if(std::abs((int)point[0] - avg[0]) < THRESHOLD[0] && std::abs((int)point[1] - avg[1]) < THRESHOLD[1] && std::abs((int)point[2] - avg[2]) < THRESHOLD[2]) {
                    mask.at<cv::Vec3b>(r, c)[0] = 92;
                    mask.at<cv::Vec3b>(r, c)[1] = 37;
                    mask.at<cv::Vec3b>(r, c)[2] = 201;
                }
                else {
                    mask.at<cv::Vec3b>(r, c) = img.at<cv::Vec3b>(r, c);
                }
            }
        }
